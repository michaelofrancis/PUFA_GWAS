...
for (i in 1:length(pheno2)){
        ...
        reformattedUKB<-MungeSumstats::format_sumstats(path=UKB2, ref_genome="GRCh37")
        ...
        write.table(refoUKB, paste(dir, "/munge/UKB/", pheno2[i],".M", m, ".txt", sep=""), quote=F, row.names=F)
        }#end model loop
} #end UKB loop
