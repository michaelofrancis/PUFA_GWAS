...
#Read UKB
#pheno<-c("w3FA_NMR_resinv","w3FA_NMR_TFAP_resinv",
...
pheno2<-c("w3FA_NMR_resinv","w6FA_NMR_resinv","DHA_NMR_resinv","LA_NMR_resinv","MUFA_NMR_resinv")
...
for (i in 1:length(pheno2)){
	...
	reformattedUKB<-MungeSumstats::format_sumstats(path=UKB2, ref_genome="GRCh37", INFO_filter=0.3)
	...
	write.table(refoUKB, paste(dir, "/munge/UKBEUR/", pheno2[i],".M", m, ".txt", sep=""), quote=F, row.names=F)
} #end UKB loop
