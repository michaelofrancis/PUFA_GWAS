...
#SBATCH --job-name=ldsc2
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=16
#SBATCH --time=144:00:00
#SBATCH --mem=180000
#SBATCH --output=ldsc2.%j.out
#SBATCH --error=ldsc2.%j.err
...
mkdir -p $outdir/LDscore/M"$m"/"$j"

python $ldscdir/ldsc.py \
--h2 $outdir/M"$m"/"$j"/"$j"_mungesumstats.sumstats.gz \
--ref-ld-chr $ldscdir/eur_w_ld_chr/ \
--w-ld-chr $ldscdir/eur_w_ld_chr/ \
--out $outdir/LDscore/M"$m"/"$j"/LDscore
