...
#Load data=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
###-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
source("/scratch/mf91122/UKB-pheno/manyColsToDummy.R")

withdrawn<-read.csv("/scratch/mf91122/UKB-pheno/w48818_20210809.csv", header = FALSE)
...
new<-beforeloop[(beforeloop$IID %in% unlist(QCids)),]
...
if(
	sum(grepl(1, 
	as.double(unlist(new[paste("center", i, sep="")]))
	)) #close sum grepl
	==0) #close if
	{
	new<-new%>%select(-paste("center", i, sep=""))
	removecenter<-c(removecenter, i)
	}
}

keepcenter<-2:22
keepcenter<-keepcenter[!keepcenter %in% removecenter]
keepcenter<-paste("center", keepcenter, sep="")

#CENTER1 do not include in regression because remove one dummy variable.
...
for (p in 1:length(phenotypes)){
    assign(paste("lm", phenotypes[p], sep="_"), 
           lm(new[[phenotypes[p]]] ~ ., data=temp_subset, 
...
for (p in 1:length(phenotypes)){
    assign(paste("lm", phenotypes[p], sep="_"),
           lm(new[[phenotypes[p]]] ~ ., data=temp_subset2,
        na.action=na.exclude))
    lmname<-paste("lm", phenotypes[p], sep="_")
    lmobj<-get(lmname)
...
outdir="/scratch/mf91122/PUFA-GWAS/UKB-multi-ancestry/pheno"
