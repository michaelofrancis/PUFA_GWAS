...
setwd("/work/kylab/mike/PUFA-GWAS/UKB-multi-ancestry/pheno")
...
#1.Genetic ethnicity = Caucasian VIA PAN UKBB xxxxNO this is multi-ancestry one.
...
#NO Filter by Genetic ethnicity = Caucasian VIA PAN UKBB
...
for (i in 1:length(popin)){

	bdkeeppop<-bd_QC[bd_QC$pop==popin[i],]
	QCkeepparticipants<-bdkeeppop%>%select(IID)

	write.table(QCkeepparticipants, file= paste("bd_QC-keep.", popin[i], ".txt",sep=""), 
